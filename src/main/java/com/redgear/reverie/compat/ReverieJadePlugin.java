...
public final class ReverieJadePlugin implements IWailaPlugin {
    ...
    private static final class Provider implements IServerDataProvider<BlockAccessor>,IBlockComponentProvider {
        ...
        @Override public void appendServerData(CompoundTag data,BlockAccessor accessor){
            ...
            data.putInt("Occupants",ReverieBedLinksData.get(level.getServer()).occupantCount(bed));
            data.putInt("Capacity",ReverieConfig.MAX_DREAMERS_PER_BED.get());
            data.putString("Policy",ReverieBedAccessData.get(level.getServer()).policy(bed).name());
            var dreamBed=ReverieBedLinksData.get(level.getServer()).dreamBed(bed);
        ...
        @Override public void appendTooltip(ITooltip tooltip,BlockAccessor accessor,IPluginConfig config){
            ...
            tooltip.add(Component.translatable("jade.reverie.access",data.getString("Policy").toLowerCase(java.util.Locale.ROOT)));
            tooltip.add(Component.translatable(data.getBoolean("Anchored")?"jade.reverie.anchored":"jade.reverie.unanchored"));
        }
    }
}
